...
Our Phase 0 baseline processor extends the original MERL-DSU framework with the following key contributions:

\subsubsection{Complete Data Forwarding Implementation}

We implemented comprehensive data forwarding logic in the Hazard Unit to resolve RAW (Read-After-Write) hazards:
...
\subsubsection{JALR and AUIPC Instruction Support}

We added full support for JALR (Jump and Link Register) and AUIPC (Add Upper Immediate to PC) instructions:

\begin{itemize}
    \item \textbf{ALUSrcA Control:} New control signal selecting between register value and PC for ALU input A
    \item \textbf{PCTargetSrc Control:} Selects between PC+Immediate (for branches/JAL) and ALU result (for JALR)
\end{itemize}

\subsubsection{Remaining Limitations for Future Phases}
...
\subsection{Phase 0 Baseline Strengths}

Our Phase 0 baseline processor demonstrates significant improvements over the original MERL-DSU framework:

\begin{enumerate}
    \item \textbf{Complete Data Forwarding Logic:} We implemented full forwarding support in the Hazard Unit with ForwardAE/ForwardBE (2-bit) for Execute stage and ForwardAD/ForwardBD (1-bit) for Decode stage. This eliminates most pipeline stalls due to RAW hazards.

    \item \textbf{Extended Instruction Support:} Added proper handling for JALR and AUIPC instructions through new control signals (ALUSrcA, PCTargetSrc), enabling PC-relative addressing and indirect jumps.

    \item \textbf{Modular and Clean Architecture:} The design maintains a highly modular structure with distinct files for each pipeline stage (Fetch\_Cycle.v, Decode\_Cycle.v, Execute\_Cycle.v, etc.). This separation makes debugging and extension significantly easier.

    \item \textbf{Comprehensive Branch Support:} The Control Unit supports all six RV32I branch types (BEQ, BNE, BLT, BGE, BLTU, BGEU) with proper signed/unsigned comparisons.
...
Phase 0 represents our baseline processor, which we developed by extending the original MERL-DSU framework with complete data forwarding and extended instruction support.

\subsubsection{Complete Hazard Unit with Data Forwarding}

We implemented a fully functional Hazard Unit with comprehensive forwarding logic:
...
\subsubsection{JALR and AUIPC Support}

We extended the Decode and Execute stages to support JALR and AUIPC instructions:

\begin{lstlisting}[language=Verilog, caption=Phase 0 - JALR/AUIPC Control Signals]
// New outputs added to Decode_Cycle module:
...
\subsubsection{Decode Stage Forwarding Multiplexers}

We added forwarding muxes in the Decode stage for branch operands:

\begin{lstlisting}[language=Verilog, caption=Phase 0 - Decode Stage Forwarding Muxes]
// Forwarding Muxes for branch comparison operands
