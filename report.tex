...
The testbench generates a 100MHz clock (10ns period) using a simple toggle mechanism. The reset sequence holds reset high for 20ns (2 clock cycles) to ensure proper initialization of all pipeline registers before execution begins.
...
\textbf{4. Branch Prediction Modeling}

The testbench implements a static ``predict not-taken'' branch prediction model. For each branch instruction, it compares the actual outcome (PCSrcE) against the prediction:
\begin{itemize}
    \item If PCSrcE = 0 (branch not taken), prediction was correct
    \item If PCSrcE = 1 (branch taken), prediction was incorrect (misprediction)
\end{itemize}

\textbf{5. Cache Simulation}

To provide realistic performance metrics, we simulate 64-entry direct-mapped caches for both instructions and data:
\begin{itemize}
    \item \textbf{I-Cache:} Indexed by PCF[9:4], tagged by PCF[31:10]. Accessed every cycle.
    \item \textbf{D-Cache:} Indexed by ALU\_ResultM[9:4], tagged by ALU\_ResultM[31:10]. Accessed only on load/store operations.
\end{itemize}

\textbf{6. Performance Report Generation}

At simulation end, the \texttt{print\_report} task calculates and displays comprehensive metrics including CPI, IPC, stalls per instruction, branch prediction accuracy, cache hit rates, and total execution time in nanoseconds.
