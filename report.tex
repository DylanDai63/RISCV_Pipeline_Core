...
\textbf{The Issue:} If an instruction tries to read a register that the immediately preceding instruction is loading from memory (e.g., \texttt{LW x1, 0(x2)} followed by \texttt{ADD x3, x1, x4}), the data is not yet available in the Execute stage. Forwarding cannot solve this because the data is still in the Data Memory.

\textbf{Evidence:} The original Hazard\_Unit.v only outputs ForwardAE and ForwardBE. It lacks outputs for StallF (freeze PC) and StallD (freeze Decode) or FlushE. Without this, the processor will execute the ADD using old/incorrect data.
...
\begin{enumerate}
    \item \textbf{Basic Data Forwarding Infrastructure:} The original Hazard Unit includes forwarding logic (ForwardAE, ForwardBE) that correctly detects RAW hazards between Execute and Memory/Writeback stages. The 4-to-1 multiplexers in the Execute stage are already in place.
...
Phase 0 represents the original MERL-DSU baseline processor without any hazard handling mechanisms.

\subsubsection{Implementation Status}

\begin{itemize}[noitemsep]
    \item \textbf{Data Forwarding:} Not implemented
