...
This section describes our phase-by-phase implementation to fix the baseline processor's hazard handling issues. Each phase builds upon the previous one, progressively adding functionality until full correctness is achieved.
...
Before implementing any changes, we analyzed the baseline MERL-DSU processor to understand its limitations.

\subsubsection{Original Hazard Unit}

The baseline Hazard Unit had minimal functionality:
...
\begin{itemize}[noitemsep]
    \item Forwarding logic did not correctly generate control signals
    \item No load-use hazard detection (ResultSrcE0, RD\_E inputs missing)
    \item No stall signals (StallF, StallD) for pipeline freezing
    \item No flush signals (FlushE, FlushD) for branch handling
\end{itemize}
...
Phase 1 focused on fixing data hazards through proper forwarding and load-use stall implementation.

\subsubsection{Execute Stage Forwarding}

We implemented complete forwarding logic with 2-bit selection signals:

\begin{lstlisting}[language=Verilog, caption=Phase 1 - Execute Stage Forwarding]
// ForwardAE/ForwardBE: 2-bit selectors for 4-to-1 mux
// 2'b00: Use register file output (no forwarding)
// 2'b01: Forward from Writeback stage (ResultW)
// 2'b10: Forward from Memory stage (ALU_ResultM) - PRIORITY
...
ForwardAD and ForwardBD (1-bit signals) forward Writeback result to Decode for branch comparisons:

\begin{lstlisting}[language=Verilog, caption=Phase 1 - Decode Stage Forwarding]
...
The lwStall signal detects when a load's destination matches a source in Decode:

\begin{lstlisting}[language=Verilog, caption=Phase 1 - Load-Use Hazard Detection]
// NEW INPUTS: ResultSrcE0 (1 if Load), RD_E (destination register)
wire lwStall;
...
\subsubsection{Fetch Stage Stall Logic}

The Fetch stage implements stalling by conditionally updating PC:

\begin{lstlisting}[language=Verilog, caption=Phase 1 - Fetch Stage Stall Implementation]
...
always @(posedge clock or posedge reset) begin
    if (reset) begin
        Inst_Fetch_reg <= 32'h0;
    end
    else if (StallF == 1'b0) begin
        Inst_Fetch_reg <= inst_fetched;  // Only update if NOT stalled
    end
    // If StallF is 1, registers retain values (implicit)
end
\end{lstlisting}

\subsubsection{Phase 1 Result}
\begin{itemize}[noitemsep]
    \item \textbf{Data Hazard:} FIXED - x6 now correctly equals 0x28
    \item \textbf{Control Hazard:} Still broken - x7 still writes 2 then 3
\end{itemize}
...
Phase 2 added branch flushing to handle control hazards when branches are taken.

\subsubsection{FlushD Signal Generation}

Branch flushing is triggered at the top level when PCSrcE indicates a taken branch:

\begin{lstlisting}[language=Verilog, caption=Phase 2 - Branch Flush Logic in design.sv]
// Top-level module (design.sv)
wire FlushD;
assign FlushD = PCSrcE; // Flush Decode when branch is taken
\end{lstlisting}

\subsubsection{Hazard Unit Update}

The Hazard Unit was extended to include PCSrcE input and FlushD output:

\begin{lstlisting}[language=Verilog, caption=Phase 2 - Extended Hazard Unit]
module Hazard_Unit(
    // ... existing inputs ...
    input PCSrcE,      // NEW: Branch taken signal
    // ... existing outputs ...
    output FlushD,     // NEW: Flush Decode stage
    output FlushE      // UPDATED: Now includes branch flush
);
    // FlushE now handles both load-use AND branch flush
    assign FlushD = PCSrcE;
    assign FlushE = lwStall | PCSrcE;
\end{lstlisting}

\subsubsection{Decode Stage Flush Handling}

The Decode\_Cycle module clears control signals on flush:

\begin{lstlisting}[language=Verilog, caption=Phase 2 - Decode Stage Flush Handling]
always @(posedge clock or posedge reset) begin
    if (reset == 1'b1) begin
        // Reset all registers to 0
    end
    else if (FlushE == 1'b1 || FlushD == 1'b1) begin
        // Flush: Clear control signals (convert to NOP)
        RegWriteD_r <= 1'b0;
        MemWriteD_r <= 1'b0;
        BranchD_r <= 3'b000;
        JumpD_r <= 1'b0;
...
\subsubsection{Branch Module}

A dedicated Branch Module handles all six RISC-V branch conditions:

\begin{lstlisting}[language=Verilog, caption=Phase 2 - Branch Comparison Module]
...
\subsubsection{PC Source Logic}

\begin{lstlisting}[language=Verilog, caption=Phase 2 - PCSrc Generation]
assign PCSrcE = JumpE | (BranchE != 3'b000 & isBranch == 1'b1);
\end{lstlisting}

\subsubsection{Phase 2 Result}
\begin{itemize}[noitemsep]
    \item \textbf{Data Hazard:} PASSED - x6 = 0x28
    \item \textbf{Control Hazard:} FIXED - x7 now writes only 3 (addi x7, 2 is flushed)
    \item \textbf{Trade-off:} Branch penalty adds cycles but ensures correctness
\end{itemize}
...
Phase 3 optimized the timing characteristics while maintaining correctness.

\subsubsection{Simplified Fetch Stage}

The Fetch stage control logic was streamlined:

\begin{lstlisting}[language=Verilog, caption=Phase 3 - Optimized Fetch Stage]
module Fetch_Cycle(
    input clock, reset, PCSrcE,
    input [31:0] PCTargetE,
    input StallF,  // Only StallF needed (FlushD handled elsewhere)
    output [31:0] InstrD, PCD, PCPlus4D
);
    // Simplified stall logic
    assign PC_Next_Final = (StallF == 1'b1) ? PCF : PC_F;
...
FlushD is now a simple combinational assignment at the top level (design.sv):

\begin{lstlisting}[language=Verilog, caption=Phase 3 - Top-Level FlushD]
// design.sv - Clean combinational flush
wire FlushD;
assign FlushD = PCSrcE;
\end{lstlisting}

\subsubsection{Final Hazard Unit}

The optimized Hazard Unit focuses on forwarding and load-use stalling:
...
module Hazard_Unit(
    ...
    input ResultSrcE0, input [4:0] RD_E,
    output [1:0] ForwardAE, ForwardBE,
    output ForwardAD, ForwardBD,
    output StallF, StallD, FlushE
);
    // Forwarding logic (unchanged from Phase 1)
    // ...

    // Load-use stall (FlushD handled at top level)
    wire lwStall;
...
\begin{itemize}[noitemsep]
    \item \textbf{Data Hazard:} PASSED - x6 = 0x28
    \item \textbf{Control Hazard:} PASSED - x7 = 3 only
    \item \textbf{Performance:} CPI improved from 1.36 to 1.25
\end{itemize}
...
Table~\ref{tab:modifications} summarizes the files modified in each phase.
...
\hline
\multirow{3}{*}{Phase 1} & Hazard\_unit.v & Added ForwardAE/BE/AD/BD, StallF/D, FlushE \\
 & Fetch\_Cycle.v & Added StallF input, conditional PC update \\
 & Decode\_Cycle.v & Added StallD input, forwarding muxes \\
\hline
\multirow{4}{*}{Phase 2} & Hazard\_unit.v & Added PCSrcE input, FlushD output \\
 & Decode\_Cycle.v & Added FlushD handling for NOP insertion \\
 & design.sv & Added FlushD = PCSrcE wire routing \\
 & Branch\_Module.v & New module for 6 branch comparisons \\
\hline
\multirow{2}{*}{Phase 3} & Fetch\_Cycle.v & Simplified control logic \\
 & Hazard\_unit.v & Streamlined stall propagation \\
\hline
\end{tabular}
